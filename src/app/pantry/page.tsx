...
import { processOfflineQueue } from '@/lib/offline/sync'; // Fixed import
...
export default function PantryPage() {
  ...
  async function verify(pantryId: number, status: 'active' | 'consumed') {
    ...
    try {
      ...
      if (!res.ok) {
        // If server error or conflict, fallback to queue
        throw new Error('Sync failed');
      }
      
      // Reload to ensure state matches server (e.g. if server applied conflict logic)
      await load();
  ...
  if (loading) return <div className="p-6 text-center text-gray-500">Loading pantry...</div>;
    ...
    <div className="p-6 max-w-2xl mx-auto space-y-6 pb-24">
      ...
      {reviewNeeded.length > 0 && (
        <section className="space-y-3 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl">
            <h2 className="text-lg font-semibold text-orange-800 mb-2">Verify Spoilage</h2>
            <p className="text-sm text-orange-600 mb-4">
              These {reviewNeeded.length} items have a low probability score based on decay rates.
            </p>
      ...
      <section className="space-y-2">
        ...
        <div className="space-y-2">
          {active.map((r) => (
            <div key={r.id} className="rounded-xl border border-gray-100 p-4 bg-white shadow-sm flex justify-between items-center">
              <div>
                <div className="font-medium text-gray-900">{r.foods?.name ?? 'Unknown'}</div>
                <div className="text-xs text-gray-500 flex items-center gap-2 mt-1">
                  <span className={`w-2 h-2 rounded-full ${
                    r.probability_score > 0.7 ? 'bg-green-500' : 'bg-yellow-500'
                  }`} />
                  Health: {Math.round(Number(r.probability_score) * 100)}%
                </div>
              </div>
              <button
